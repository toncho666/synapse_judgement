...
export interface Session {
  ...
  targetPrice: number;
  currentPrice: number;
  accuracy?: number; // calculated after some time
}

const STORAGE_KEYS = {
  USER: 'synapse_user',
  SESSIONS: 'synapse_sessions',
  REFERRALS: 'synapse_referrals',
};
...
const MOCK_SESSIONS: Session[] = [
  ...
  },
  {
    id: 's2',
    ticker: 'TSLA',
    agents: ['Technical', 'Earning Calls'],
    verdict: 'HOLD',
    confidence: 72,
  ...
  },
  {
    id: 's3',
    ticker: 'NVDA',
    agents: ['Technical', 'Fundamental', 'Portfolio', 'News', 'Earning Calls'],
    verdict: 'BUY',
    confidence: 91,
    timestamp: '2026-01-18T16:45:00Z',
    cost: 30,
    targetPrice: 1180.00,
    currentPrice: 1042.10,
    accuracy: 96,
  },
];
...
export function login(email: string, password: string): Promise<User> {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      // Demo: any email/password works, returns demo user
...
export function addSession(session: Omit<Session, 'id' | 'timestamp'>): Session {
...
export function updateCredits(userId: string, delta: number): User | null {
  const user = getUser();
  if (!user) return null;
  user.credits += delta;
  localStorage.setItem(STORAGE_KEYS.USER, JSON.stringify(user));
...
export function upgradePlan(plan: 'pro' | 'enterprise'): User | null {
  ...
  if (!user) return null;
  user.plan = plan;
  if (plan === 'pro') user.credits += 500;
  if (plan === 'enterprise') user.credits += 2000;
  localStorage.setItem(STORAGE_KEYS.USER, JSON.stringify(user));
  return user;
}
